...
router.post("/", async function (req, res, next) {
  ...
  try {
    ...
    if (!pushSub || pushError) {
      console.warn("푸시 구독 정보 없음:", pushError?.message);
      return res.redirect("/reservation/complete");
    }

    // 3. 푸시 페이로드 구성
    const payload = JSON.stringify({
      title: "예약 완료!",
      body: `${date} ${time} 예약이 정상적으로 접수되었습니다.`,
    ...
    res
      .status(500)
      .render("reservation", { title: "예약하기", user, error: err.message });
  }
});
